...
///
///
///   is a DOM interface implemented by objects that can receive
/// events and may have listeners for them.
///
///  [Element], [Document], and [Window] are the most common event
/// targets, but other objects can be event targets, too. For example
...
///
///  You can create a new object using the
/// [AbortController.AbortController()] constructor. Communicating
/// with a DOM request is done using an [AbortSignal] object.
@experimental
@JS()
class AbortController {
  external factory AbortController();

  ///  Returns a [AbortSignal] object instance, which can be used to
  /// communicate with/abort a DOM request.
  external AbortSignal get signal;

  ///  Aborts a DOM request before it has completed. This is able to
  /// abort fetch requests, consumption of any response [Body], and
  /// streams.
  /// controller.abort();
...
/// communicate with a DOM request (such as a Fetch) and abort it if
...
///
///
///  The [ChildNode] mixin contains methods and properties that are
/// common to all types of [Node] objects that can have a parent.
...
abstract class ChildNode {
  ...
  ///  Replaces this [ChildNode] in the [children] list of its parent
  /// with a set of [Node] or [DOMString] objects. [DOMString] objects
...
///  Although is not an [Array], it is possible to iterate over it
/// with [forEach()]. It can also be converted to a real [Array]
/// using [Array.from()].
///
...
class NodeList {
  ...
  external Node operator [](int index);
...
class MutationObserver {
  ...
  ///  MutationObserver, once it has been set up, passing it a target element
  ///  and a MutationObserverInit options object.
  ///
...
/// [options] parameter on the [MutationObserver.observe()] method.
@anonymous
@JS()
class MutationObserverInit {
  ///  Set to [true] to monitor the target node (and, if [subtree] is
  /// [true], its descendants) for the addition of new child nodes or
  /// removal of existing child nodes. The default value is [false].
  external bool get childList;
  external set childList(bool newValue);

  ///  Set to [true] to watch for changes to the value of attributes on
  /// the node or nodes being monitored. The default value is [true] if
  /// either of [attributeFilter] or [attributeOldValue] is specified,
  ...
  /// character data contained within the node or nodes. The default
  /// value is [true] if [characterDataOldValue] is specified,
  /// otherwise the default value is [false].
  external bool get characterData;
  external set characterData(bool newValue);

  ///  Set to [true] to extend monitoring to the entire subtree of
  /// nodes rooted at [target]. All of the other [MutationObserverInit]
  /// properties are then extended to all of the nodes in the subtree
  /// instead of applying solely to the [target] node. The default
  /// value is [false].
  ...
  /// changes when monitoring the node or nodes for attribute changes;
  /// see Monitoring attribute values in MutationObserver for details
  /// on watching for attribute changes and value recording. The
  /// default value is [false].
  external bool get attributeOldValue;
  external set attributeOldValue(bool newValue);

  ///  Set to [true] to record the previous value of a node's text
  /// whenever the text changes on nodes being monitored. For details
  /// and an example, see Monitoring text content changes in
  /// MutationObserver. The default value is [false].
  external bool get characterDataOldValue;
  external set characterDataOldValue(bool newValue);

  ///  An array of specific attribute names to be monitored. If this
  /// property isn't included, changes to all attributes cause mutation
  /// notifications.
...
class Node extends EventTarget {
  ...
  /// Returns a [DOMString] containing the name of the [Node]. The
  ...
  /// Returns a [DOMString] representing the base URL of the document
  ///   containing the [Node].
  ...
  /// shadow root if it is
  ///   available.
  /// var root = node.getRootNode(options);
  external Node getRootNode([GetRootNodeOptions? options]);

  ///  Returns a [Node] that is the parent of this node. If there is no
  /// such
  ...
  ///  Returns a [Boolean] indicating whether or not the element has
  /// any child
  ///   nodes.
  ...
  /// node
  ///    (including elements, text and comments). [NodeList] being live
  /// means that
  ...
  ///  Returns a [Node] representing the last direct child node of the
  /// node,
  ///   or [null] if the node has no child.
  ...
  ///  Clone a [Node], and optionally, all of its contents. By default,
  /// it
  ...
  ///    subtree—including text that may be in child Text nodes—is also
  ///    copied.
  ///
  ///   If false, only node will be cloned. Any text
  ///    that node contains is not cloned, either (since text is
  ///    contained by one or more child Text nodes).
  ///
  ///   deep has no effect on empty elements (such as
  ///    the <img> and <input> elements).
  ///
  ///
  ///    *Note: In the DOM4 specification (since Gecko 13.0
  ///     (Firefox 13 / Thunderbird 13 / SeaMonkey 2.10)), the optional deep argument defaults
  ///     to true.
  ///
  ///    This behavior has been changed in the latest spec!
  ///     Although deep it still optional, it now defaults to
  ///     false.
  ///
  ///    You should always provide an explicit value for backward and
  ///      forward compatibility.
  ///
  ///
  ///     With Gecko 28.0 (Firefox 28 / Thunderbird 28 / SeaMonkey 2.25 / Firefox OS 1.3)), the console warned developers not to
  ///      omit the argument.
  ///     Starting with Gecko 29.0 (Firefox 29 / Thunderbird 29 / SeaMonkey 2.26)), a shallow clone is defaulted
  ...
  ///  Returns a [Boolean] which indicates whether or not two nodes are
  /// of the
  ...
  ///  Accepts a prefix and returns the namespace URI associated with
  /// it on the given node
  ...
  ///   an optional parameter. You must explicitly pass a Node or
  ///   null. Failing to provide it or passing invalid values may behave differently in
  ...
  ///  immediately become unusable and irretrievable, and will usually be automatically deleted from
  ...
  ///  handler invoked in the course of trying to remove the element (e.g., blur.)
  ///
...
/// the DOM tree. The DOM tree includes elements such as [<body>] and
/// [<table>], among many others. It provides functionality globally
/// to the document, like how to obtain the page's URL and create new
...
class Document extends Node {
  ...
  ///  HTMLCollection both in Gecko and Internet Explorer. Opera returns a
  ...
  ///  is the returned value a pure NodeList. See bug 14869 for details.
  ...
  ///  Element.getElementsByTagNameNS for details.
  ...
  ///
  /// The following example goes through all nodes in the body, reduces the set of nodes to
  ///  elements, passes through as acceptable each node (it could reduce the set in the
  ///  acceptNode() method instead), and then makes use of tree walker iterator
  ...
  external dynamic get rootElement;
...
/// methods which are not dependent on any particular document. Such
...
class DOMImplementation {
  ...
  ///  Returns a [Boolean] indicating if a given feature is supported
  /// or not. This function is unreliable and kept for compatibility
  /// purpose alone: except for SVG-related queries, it always returns
...
/// parent. It is used as a lightweight version of [Document] that
/// stores a segment of a document structure comprised of nodes just
/// like a standard document. The key difference is due to the fact
/// that the document fragment isn't part of the active document tree
...
/// node of a DOM subtree that is rendered separately from a
...
class ShadowRoot extends DocumentFragment {
  ...
  /// when the shadow was attached (see [Element.attachShadow()]).
  @deprecated
...
/// elements. More specific classes inherit from . For example, the
/// [HTMLElement] interface is the base interface for HTML elements,
/// while the [SVGElement] interface is the basis for all SVG
/// elements. Most functionality is specified further down the class
/// hierarchy.
///
...
class Element extends Node {
  ...
  external set className(covariant dynamic newValue);
  ...
  ///  Returns a [Boolean] indicating if the element has one or more
  ...
  ///   browser's page inspector (Chrome,
  ///   Edge,
  ///   Firefox, Safari).
  ///
  ...
  ///   values. All that matters is that if the attribute is present at all, regardless of
  ///    its actual value, its value is considered to be true. The absence
  ...
  ///  Returns a [Boolean] indicating if the element has the specified
  /// attribute or not.
  ...
  ///  Returns a [Boolean] indicating whether or not the element would
  /// be selected by the specified selector string.
...
///
///
///  The [NamedNodeMap] interface represents a collection of [Attr]
/// objects. Objects inside a are not in any particular order, unlike
/// [NodeList], although they may be accessed by an index as in an
/// array.
///
///  A object is live and will thus be auto-updated if changes are
/// made to its contents internally or elsewhere.
///  Although called , this interface doesn't deal with [Node]
...
/// attributes as an object. In most DOM methods, you will directly
/// retrieve the attribute as a string (e.g.,
/// [Element.getAttribute()]), but certain functions (e.g.,
/// [Element.getAttributeNode()]) or means of iterating return types.
///
///  Warning: Starting in Gecko 7.0 (Firefox 7.0 / Thunderbird 7.0 /
/// SeaMonkey 2.4), a number of deprecated properties and methods
/// output warning messages to the console. You should revise your
...
class Attr extends Node {
  ...
  ///  A [DOMString] representing the namespace prefix of the
  /// attribute, or [null] if no prefix is specified.
  external String? get prefix;
  ...
  /// that since you get an [Attr] object from an [Element], you should
  /// already know the associated element.
  ///   As that doesn't hold true in cases like [Attr] objects being
  /// returned by [Document.evaluate], the DOM Living Standard
  /// reintroduced the property.
  ///
  ///   Gecko outputs a deprecation note starting from Gecko 7.0
  /// (Firefox 7.0 / Thunderbird 7.0 / SeaMonkey 2.4). This note was
  /// removed again in Gecko 49.0 (Firefox 49.0 / Thunderbird 49.0 /
  /// SeaMonkey 2.46).
  ///
  ///
  external Element? get ownerElement;

  ///  This property always returns [true]. Originally, it returned
  /// [true ]if the attribute was explicitly specified in the source
  /// code or by a script, and [false] if its value came from the
  /// default one defined in the document's DTD.
...
/// object that contains characters. This is an abstract interface,
/// meaning there aren't any objects of type : it is implemented by
/// other interfaces like [Text], [Comment], or
...
class CharacterData extends Node {
  ...
  /// contained in [CharacterData.data].
  external int get length;

  ///  Returns a [DOMString] containing the part of
  /// [CharacterData.data] of the specified length and starting at the
  /// specified offset.
  ///  domString = characterData.substringData(offset, count)
  ///
  external String substringData(int offset, int count);

  ///  Appends the given [DOMString] to the [CharacterData.data]
  /// string; when this method returns, [data] contains the
  ...
  /// method returns, [data] contains the modified [DOMString].
...
///
///
///  The interface represents the textual content of [Element] or
/// [Attr].
///
///  If an element has no markup within its content, it has a single
/// child implementing that contains the element's text. However, if
/// the element contains markup, it is parsed into information items
/// and nodes that form its children.
///
///  New documents have a single node for each block of text. Over
/// time, more nodes may be created as the document's content
/// changes. The [Node.normalize()] method merges adjacent objects
/// back into a single node for each block of text.
///
///
@JS()
...
/// ]
///
///  The only sequence which is not allowed within a CDATA section is
...
/// ]
///
///  Note that CDATA sections should not be used within HTML; they
/// only work in XML.
///
///
@JS()
...
/// instruction; that is, a [Node] which embeds an instruction
/// targeting a specific application but that can be ignored by any
/// other applications which don't recognize the instruction.
///
/// A processing instruction is different from the XML declaration.
///  User-defined processing instructions cannot begin with "[xml]",
/// as [xml]-prefixed processing-instruction target names are
/// reserved by the XML specification for particular, standard uses
/// (see, for example, [<?xml-stylesheet ?>]).
/// The interface inherits methods and properties from [Node].
///
///
@JS()
...
/// are represented in HTML and XML as content between '[<!--]' and
/// '[-->]'. In XML, the character sequence '[--]' cannot be used
/// within a comment.
///
///
@JS()
...
/// to reflect changes which occur within the DOM tree. It offers the
/// same set of properties and methods as [AbstractRange].
///
...
class NodeIterator {
  ...
  ///        only when creating a [NodeIterator] with an [Attr] node as
  /// its root; in this case, it means that
  ///        the attribute node will appear in the first position of
  /// the
  ///        iteration or traversal. Since attributes are never
  /// children of
  ///        other nodes, they do not appear when traversing over the
  /// document
  /// 						tree.
  ///
  ///
  ...
  ///        creating a [NodeIterator] with an [Entity] node as its
  /// root; in this case, it means that
  /// 						the [Entity] node will appear in the first position
  ///        of the traversal. Since entities are not part of the
  /// document
  ///        tree, they do not appear when traversing over the document
  /// tree.
  ///
  ///
  ///
  ...
  ///        when creating a [NodeIterator] with a [Notation] node as
  /// its root; in this case, it means
  /// 						that the [Notation] node will appear in the first
  ///        position of the traversal. Since entities are not part of
  /// the
  ...
  ///  This operation is a no-op. It doesn't do anything. Previously it
  /// was telling the
...
class TreeWalker {
  ...
  ///
  ///   [32]
  ///    Shows [Entity] nodes. This is meaningful only when creating a
  /// [TreeWalker] with an [Entity] node as its root; in this case, it
  /// means that the [Entity] node will appear in the first position of
  /// the traversal. Since entities are not part of the document tree,
  ...
  ///
  ///   [2048]
  ///    Shows [Notation] nodes. This is meaningful only when creating
  /// a [TreeWalker] with a [Notation] node as its root; in this case,
  /// it means that the [Notation] node will appear in the first
  ...
  ///  Moves the current [Node] to the first visible ancestor node in
  /// the document order, and returns the found node. It also moves the
  ...
  ///  Moves the current [Node] to the first visible child of the
  /// current node, and returns the found child. It also moves the
  /// current node to this child. If no such child exists, returns
  /// [null] and the current node is not changed.
  ...
  ///  Moves the current [Node] to the last visible child of the
  /// current node, and returns the found child. It also moves the
  /// current node to this child. If no such child exists, [null] is
  /// returned and the current node is not changed.
  ...
  ///  Moves the current [Node] to its previous sibling, if any, and
  /// returns the found sibling. If there is no such node, return
  /// [null] and the current node is not changed.
  ...
  ///  Moves the current [Node] to its next sibling, if any, and
  /// returns the found sibling. If there is no such node, [null] is
  /// returned and the current node is not changed.
  ...
  ///  Moves the current [Node] to the previous visible node in the
  ...
  ///  Moves the current [Node] to the next visible node in the
  /// document order, and returns the found node. It also moves the
  /// current node to this one. If no such node exists, returns [null]
...
class NodeFilter {
  ...
  ///
  /// 				[FILTER_ACCEPT]
  ///      Value returned by the [NodeFilter.acceptNode()] method when
  /// a node should be accepted.
  ///
  ///
  /// 				[FILTER_REJECT]
  ///
  ///      Value to be returned by the [NodeFilter.acceptNode()] method
  /// when a node should be rejected. For [TreeWalker], child nodes are
  /// also rejected.
  ///
  ///      For [NodeIterator], this flag is synonymous with
  /// [FILTER_SKIP].
  ///
  ///
  ///
  /// 				[FILTER_SKIP]
  ///
  ///      Value to be returned by [NodeFilter.acceptNode()] for nodes
  /// to be skipped by the [NodeIterator] or [TreeWalker] object.
  ///
  ///      The children of skipped nodes are still considered. This is
  /// treated as "skip this node but not its children".
  ///
  ///
  ///
  ///
...
/// [HTMLLinkElement.relList], [HTMLAnchorElement.relList],
/// [HTMLAreaElement.relList], [HTMLIframeElement.sandbox], or
/// [HTMLOutputElement.htmlFor]. It is indexed beginning with [0] as
/// with JavaScript [Array] objects. is always case-sensitive.
@JS()
class DOMTokenList {
...
/// XPath expression within the context of a given node. Since XPath
/// expressions can result in a variety of result types, this
/// interface makes it possible to determine and handle the type and
/// value of the result.
@JS()
class XPathResult {
...
/// from its DOM tree. This is useful when an expression will be
/// reused in an application, because it is just compiled once and
/// all namespace prefixes which occur within the expression are
/// preresolved.
///
